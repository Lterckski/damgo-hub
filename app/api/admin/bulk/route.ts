...
export async function POST(request: Request) {
  ...
  if (ids.length > 200) {
    return NextResponse.json({ error: "Select at most 200 records at a time" }, { status: 400 });
  }

  try {
    const result = await applyBulkAction(guard.context, action, ids as string[], value, reason);
