...
export async function POST(request: Request) {
  ...
  if (
    audience === "ROLE" &&
    audienceRole !== "org:admin" &&
    audienceRole !== "org:member"
  ) {
    return NextResponse.json(
      { error: "Pick a role to send to" },
      { status: 400 },
    );
  }
  if (audience === "PROJECT" && typeof projectId !== "string") {
    return NextResponse.json(
      { error: "Pick a project to send to" },
      { status: 400 },
    );
  }
  if (audience === "MEMBER" && typeof audienceMemberId !== "string") {
    return NextResponse.json(
      { error: "Pick a member to send to" },
      { status: 400 },
    );
  }
  ...
  try {
    const result = await sendBroadcast(guard.context.actor, {
      ...
      audienceMemberId:
        typeof audienceMemberId === "string" ? audienceMemberId : null,
    });
      ...
      return NextResponse.json(
        {
          ok: true,
          ...result,
          message: "Sent, but that audience currently has no members",
        },
        { status: 200 },
      );
    }
