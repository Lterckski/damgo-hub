...
export async function GET(request: Request) {
  ...
  const projects = await prisma.project.findMany({
    where: { ...where, AND: [await entityVisibilityWhere("project")] },
...
export async function POST(request: Request) {
  ...
  if (typeof ownerId === "string" && ownerId !== creator.id) {
    const ownerCandidate = await prisma.member.findUnique({
      where: { id: ownerId },
    });
    if (!ownerCandidate) {
      return NextResponse.json(
        { error: "ownerId is not a real member" },
        { status: 400 },
      );
    }
  ...
  const collaboratorIds = ((memberIds as string[]) ?? []).filter(
    (id) => id !== resolvedOwnerId,
  );
  ...
  const project = await prisma.project.create({
    data: {
      ...
      category:
        typeof category === "string" ? (category as ProjectCategory) : null,
