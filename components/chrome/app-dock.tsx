 ...
 *
 * On desktop, collapsed by default — a small handle is the only persistent hint it's
 ...
 * visible. Phones and coarse pointers use a visible button and labeled menu.
 * Keyboard users get the same reveal via focus-within, so tabbing
    ...
    <>
      <div className="mobile-navigation">
        <Dialog open={mobileOpen} onOpenChange={setMobileOpen}>
          <DialogTrigger
            render={<Button className="min-h-11 rounded-full px-5 shadow-lg" />}
          >
            <Menu className="h-5 w-5" /> Navigate
          </DialogTrigger>
          <DialogContent className="rounded-3xl">
            <DialogTitle>Navigate Damgo Hub</DialogTitle>
            ...
            >
              {items.map((item) => (
                <Link
                  ...
                  className={cn(
                    "flex min-h-14 items-center gap-2 rounded-xl px-3 py-3 text-sm font-medium outline-none focus-visible:ring-2 focus-visible:ring-brand",
                    isActive(item.href)
      ...
      <div className="desktop-navigation group/dock fixed inset-x-0 bottom-0 z-50 flex h-6 justify-center">
        <div
          aria-hidden="true"
