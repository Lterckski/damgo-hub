 ...
 */
export function RecordDrawer({ target, onClose, buildActions, refreshToken }: RecordDrawerProps) {
  return (
...
function Loader({
  ...
  React.useEffect(() => {
    ...
    fetch(`/api/admin/records/${kind}/${recordId}`, { signal: controller.signal })
      .then(async (response) => {
        const payload: unknown = await response.json().catch(() => null);
        const data = (payload ?? {}) as Record<string, unknown>;
        if (!response.ok) throw new Error(typeof data.error === "string" ? data.error : "Failed to load");
    ...
    <>
        ...
        {detail && (
          <>
            ...
            {detail.kind === "finance" && <TransactionBody transaction={detail.transaction} />}
            {detail.kind === "penalties" && <PenaltyBody penalty={detail.penalty} />}
            {detail.kind === "projects" && <ProjectBody project={detail.project} />}
                ...
                <DrawerBody>
                  <Section title="Excerpt">
                    <p className="whitespace-pre-wrap text-sm text-copy-secondary">
                      {detail.doc.excerpt ?? "This document has no text content yet."}
                    </p>
                  </Section>
                  <Section title="Open">
                    <a
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="mb-6 last:mb-0">
      <h3 className="mb-2 text-xs font-bold tracking-[0.08em] text-copy-primary uppercase">{title}</h3>
      {children}
    </section>
  );
}
    ...
    <ul className="flex flex-col gap-1.5">
      {items.map((item) => (
        ...
        >
          ...
          {item.meta && <p className="mt-0.5 text-xs text-copy-muted">{item.meta}</p>}
        </li>
      ))}
    </ul>
  );
...
function StatTile({ label, value, hint }: { label: string; value: string; hint?: string }) {
  return (
    <div className="rounded-xl bg-base px-3 py-2.5 ring-1 ring-surface-border">
      <p className="text-[10px] font-bold tracking-[0.08em] text-copy-secondary uppercase">{label}</p>
      <p className="mt-0.5 text-lg font-bold text-copy-primary tabular-nums">{value}</p>
      ...
      <DrawerHeader className="border-b border-surface-border px-6 pt-6 pb-4">
        <div className="flex items-start gap-3 pr-10">
          ...
          <div className="min-w-0">
            ...
            <DrawerDescription className="truncate">{member.email}</DrawerDescription>
            <div className="mt-2 flex flex-wrap gap-1.5">
              {member.isLeader && <Badge>Leader</Badge>}
              {member.orgRole === "org:admin" && !member.isLeader && <Badge>Assistant Leader</Badge>}
              <Badge variant={member.status === "ACTIVE" ? "default" : "secondary"}>
                {MEMBER_STATUS_LABEL[member.status as keyof typeof MEMBER_STATUS_LABEL] ?? member.status}
              </Badge>
              {!member.inClerkOrg && <Badge variant="destructive">Not in Clerk org</Badge>}
            </div>
          </div>
        </div>
      </DrawerHeader>
      ...
      <DrawerBody>
        <Section title="At a glance">
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <StatTile label="Open tasks" value={String(member.stats.openTasks)} />
            <StatTile label="Completed" value={String(member.stats.completedTasks)} />
            <StatTile
              label="Owed"
        ...
        <Section title="Penalties">
          <RelatedList items={member.penalties} empty="No penalties on record." />
        </Section>

        <Section title="Contributions">
          <RelatedList items={member.contributions} empty="No approved transactions." />
        </Section>

        <Section title="Recent admin activity">
          <RelatedList items={member.recentActivity} empty="Nothing recorded against this member yet." />
        </Section>
      </DrawerBody>
    </>
  );
  ...
  return (
    <>
      <DrawerHeader className="border-b border-surface-border px-6 pt-6 pb-4">
        ...
        <DrawerDescription>
          {transaction.type === "INCOME" ? "Income" : "Expense"} from {transaction.memberName}
        </DrawerDescription>
      </DrawerHeader>
      <DrawerBody>
          ...
          <Facts
            rows={[
              ...
              ...(transaction.penaltyReason
                ? ([["From penalty", transaction.penaltyReason]] as [string, React.ReactNode][])
                : []),
            ]}
          />
        </Section>
        <Section title="History">
          <RelatedList items={transaction.history} empty="No admin actions on this record yet." />
        </Section>
      </DrawerBody>
    </>
  );
        ...
        <Section title="Details">
          <Facts
            rows={[
              [
                "Status",
                <Badge key="s" variant={penalty.isPastDue ? "destructive" : "default"}>
                  {penalty.isPastDue ? "OPEN · PAST DUE" : penalty.status}
                </Badge>,
              ],
              ["Amount", penalty.amountCents === null ? "Non-monetary" : formatPHP(penalty.amountCents)],
              [
                "Due",
              ...
              [
                "Resolved",
                penalty.resolvedAt ? new Date(penalty.resolvedAt).toLocaleDateString() : "—",
              ],
              ["Ledger entry", penalty.linkedTransactionId ? "Created on resolution" : "None"],
            ]}
          />
        </Section>
        <Section title="History">
          <RelatedList items={penalty.history} empty="No admin actions on this record yet." />
        </Section>
      </DrawerBody>
    </>
  );
        ...
        <Section title="Details">
          <Facts
            rows={[
              ...
              ["Category", project.category ? readableRole(project.category) : "—"],
              [
                "Budget",
                project.estimatedBudgetCents === null ? "—" : formatPHP(project.estimatedBudgetCents),
              ],
              [
                "Timeline",
                [
              ...
              ["Last activity", new Date(project.updatedAt).toLocaleDateString()],
            ]}
          />
        </Section>
        {project.objectives && (
          <Section title="Objectives">
            <p className="text-sm whitespace-pre-wrap text-copy-secondary">{project.objectives}</p>
          </Section>
        )}
        <Section title="Collaborators">
          <RelatedList items={project.members} empty="No collaborators beyond the owner." />
        </Section>
        <Section title="Tasks">
          <RelatedList items={project.tasks} empty="No tasks on this project." />
        </Section>
        <Section title="History">
          <RelatedList items={project.history} empty="No admin actions on this record yet." />
        </Section>
      </DrawerBody>
    </>
  );
      ...
      <DrawerBody>
        <Section title="Details">
          <Facts
            rows={[
              ...
              [
                "Due",
                <span key="d" className={task.isOverdue ? "font-semibold text-state-error" : undefined}>
              ...
              [
                "Assignees",
                task.assignees.length ? task.assignees.map((a) => a.displayName).join(", ") : "Unassigned",
              ],
            ]}
          />
        </Section>
        {task.description && (
          <Section title="Description">
            <p className="text-sm whitespace-pre-wrap text-copy-secondary">{task.description}</p>
          </Section>
        )}
        <Section title="History">
          <RelatedList items={task.history} empty="No admin actions on this record yet." />
        </Section>
      </DrawerBody>
    </>
  );
