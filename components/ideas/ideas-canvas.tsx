...
export function IdeasCanvas({ currentMemberId }: { currentMemberId: string }) {
  const { nodes, edges, onNodesChange, onEdgesChange } = useLiveblocksFlow<IdeaNodeType>({
    suspense: true,
    nodes: { initial: [] },
    edges: { initial: [] },
  });
  ...
  function addIdea() {
    ...
    const position = { x: 80 + Math.random() * 320, y: 80 + Math.random() * 240 };
  ...
  function commitText(nodeId: string, text: string) {
    ...
    if (!node) return;
    onNodesChange([{ id: nodeId, type: "replace", item: { ...node, data: { ...node.data, text } } }]);
  }
