...
- **Planned header extension (awaiting confirmation)**: [unit 23](feature-specs/23-global-search-notifications-header.md) specifies logo + Clerk org switcher, centered desktop search with shortcut hint, and quick create/bell/avatar controls. Mobile shows logo/search icon/bell/avatar with the other utilities in the account menu. Include visible org/effective role, stable bell/avatar skeletons, accessible palette focus management and live unread-count announcements. Preserve the bottom dock and the existing `main` scroll container; scrolled header styling must observe that container.
- **Planned theme control (not yet implemented)**: unit 23 adds persisted System/Light/Dark selection through the avatar menu. System remains the default; existing palette/typography tokens are unchanged. The system-only description above remains accurate until the explicit override lands.
