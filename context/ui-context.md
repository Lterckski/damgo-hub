...
Unresolved decisions on the single-activation requirement above, recorded 2026-09-07.

- **What counts as an action button?** Mutations clearly. Does it also cover navigation (Previous/Next, tab switches, row-open), read-only toggles (filters, column visibility, theme), and controls that open a dialog without writing anything?
- **One shared control or a per-usage guard?** A shared wrapper — an `ActionButton` in `components/shared/` that owns the ref guard, busy label, and error restore — would make the rule enforceable in review, but `components/ui/button.tsx` is generated and off-limits, so the wrapper would have to be adopted at every call site. Retrofit every existing action to it, or document the ref pattern and apply it case by case?
- **How far does server-side idempotency go?** Status-conflict rejection (`409` on an already-decided record) covers decisions, but creates — a new task, a new expense, a new proposal — have no natural conflict to detect. Do those need client-supplied idempotency keys, or is the synchronous client guard accepted for them?
- **What feedback, and for how long?** Should a fast action show a busy state at all (a spinner that flashes for 80 ms reads as a glitch), and is there a minimum visible duration, or does the control simply stay disabled until the response lands?
- **Do confirmation dialogs count as satisfying the rule?** Delete and the Danger Zone actions already require a typed confirmation or a reason. Is that sufficient protection for those, or do their confirm buttons need the same guard as everything else?
