...
| 2 | The common API guard hydrated the full viewer on every request | All 69 guarded handler methods called `hubApiGuard()`, which fetched Clerk's whole roster, reconciled members, rewrote `HubMembership`, and loaded project membership before the handler's own auth/data work. `requireWorkspaceSession()` also made a separate Clerk membership API call despite the verified session already carrying `orgId`/`orgRole`. | **150–600 ms** on ordinary API calls after region alignment; materially more with the old cross-region DB placement | `hubApiGuard()` now checks only the workspace/session boundary. `getHubViewer()` uses the signed-in member and project access directly. Workspace authorization uses verified Clerk session claims; full roster projection runs only for features that need the roster. |
...
| `lib/clerk-roster.ts` | Clerk membership pages fetched one after another, separately per consumer | First page supplies `totalCount`; remaining pages run together and the result is shared within the render. Invitations are also shared. |
...
| --- | --- | --- |
| `lib/hub/context.ts` | Delete plus one `HubMembership.upsert` per org member on every viewer request | Fixed. Removed from viewer hot path; roster projection uses delete + `createMany`. |
